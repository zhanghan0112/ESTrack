...
def padding(ori, h, w, ch, pad_val, top_pad, bottom_pad, left_pad, right_pad,
            xmin, xmax, ymin, ymax):
    if any([top_pad, bottom_pad, left_pad, right_pad]):
        size = (h + top_pad + bottom_pad, w + left_pad + right_pad, ch)
        tmp = np.zeros(size, np.uint8)
        if isinstance(ori, torch.Tensor):
            size = (h + top_pad + bottom_pad, w + left_pad + right_pad)
            tmp = torch.zeros(size, dtype=ori.dtype, device=ori.device)
        tmp[top_pad:top_pad + h, left_pad:left_pad + w, ...] = ori
        if top_pad:
            tmp[0:top_pad, left_pad:left_pad + w, ...] = pad_val
        if bottom_pad:
            tmp[h + top_pad:, left_pad:left_pad + w, ...] = pad_val
        if left_pad:
            tmp[:, 0:left_pad, ...] = pad_val
        if right_pad:
            tmp[:, w + left_pad:, ...] = pad_val
        patch = tmp[int(ymin):int(ymax + 1),
                int(xmin):int(xmax + 1), ...]
    else:
        patch = ori[int(ymin):int(ymax + 1),
                int(xmin):int(xmax + 1), ...]
    return patch
...
def sample_target(im, target_bb, search_area_factor, output_sz=None, mask=None):
    ...
    #                int(context_xmin):int(context_xmax + 1), :]
    im_patch = padding(im, h, w, ch, avg_chans, top_pad, bottom_pad, left_pad, right_pad,
                       context_xmin, context_xmax, context_ymin, context_ymax)
    if mask is not None:
        mask_patch = padding(mask, h, w, ch, 0, top_pad, bottom_pad, left_pad, right_pad,
                             context_xmin, context_xmax, context_ymin, context_ymax)
    ...
    mask_patch = F.interpolate(mask_patch[None, None], (output_sz, output_sz), mode='bilinear', align_corners=False)[0, 0]
    # mask_patch = cv.resize(mask_patch, (output_sz, output_sz))
    return im_patch, resize_factor, mask_patch

def sample_target_transconv(im, target_bb, search_area_factor, output_sz=None, mask=None):
    """ Extracts a square crop centered at target_bb box, of area search_area_factor^2 times target_bb area
...
def jittered_center_crop(frames, box_extract, box_gt, search_area_factor, output_sz, masks=None):
    ...
    '''Use normalize coord here'''
    box_crop = [transform_image_to_crop(a_gt, a_ex, rf, crop_sz, normalize=False)
