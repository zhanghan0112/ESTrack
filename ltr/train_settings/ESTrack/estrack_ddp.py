...
import ltr.models.tracking.ESTrack as ESTrack
...
def run(settings):
    ...
    data_processing_train = processing.siamProcessing(search_area_factor=settings.search_area_factor,
    ...
    data_processing_val = processing.siamProcessing(search_area_factor=settings.search_area_factor,
                                                    template_area_factor=settings.template_area_factor,
                                                    search_sz=settings.search_sz,
                                                    temp_sz=settings.temp_sz,
                                                    center_jitter_factor=settings.center_jitter_factor,
    ...
    #train mask
    dataset_train = sampler.ESTrackSampler([lasot_train,got10k_train,trackingnet_train,coco_train], [1,1,1,1],
    ...
    dataset_val = sampler.ESTrackSampler([got10k_val], [1], samples_per_epoch=2000, max_gap=200,num_template_frames=1,
                                         num_search_frames=1,frame_sample_mode='causal',response_map=settings.response_map, processing=data_processing_val,neg=settings.neg_sample)
    ...
    model = ESTrack.ESTrack_resnet50(backbone_pretrained=True,head_name="ESTrack")
    ...
    param_dicts = [
        {"params": filter(lambda x: x.requires_grad, model.module.backbone.parameters()), "lr":1e-4},
        {"params": model.module.neck_z_cls.parameters()},
        {"params": model.module.neck_z_loc.parameters()},
        {"params": model.module.neck_x_cls.parameters()},
        {"params": model.module.neck_x_loc.parameters()},
