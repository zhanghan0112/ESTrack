...
import ltr.models.tracking.siamyolo_model as siam
...
from ltr.models.loss.siamLoss import SigmoidCrossEntropyRetina
...
def run(settings):
    ...
    data_processing_train = processing.siamyolov1Processing(search_area_factor=settings.search_area_factor,
    ...
    data_processing_val = processing.siamyolov1Processing(search_area_factor=settings.search_area_factor,
                                                    template_area_factor=settings.template_area_factor,
                                                    search_sz=settings.search_sz,
                                                    temp_sz=settings.temp_sz,
                                                    center_jitter_factor=settings.center_jitter_factor,
    ...
    dataset_train = sampler.siamyoloSampler([lasot_train,got10k_train,trackingnet_train,coco_train], [1,1,1,1],
    ...
    dataset_val = sampler.siamyoloSampler([got10k_val], [1], samples_per_epoch=2000, max_gap=200,num_template_frames=1,
                                         num_search_frames=1,frame_sample_mode='causal',response_map=13,processing=data_processing_val,neg=settings.neg_sample)
    ...
    loader_val = LTRLoader('val', dataset_val, training=False, batch_size=settings.batch_size,
                          num_workers=settings.num_workers, shuffle=shuffle, drop_last=True, stack_dim=0, sampler=val_sampler, epoch_interval=10)
    ...
    model = siam.siamyolov1_repvgg(neck_in_channel=192,neck_out_channels=256)
    ...
    objective = {'clsloss':SigmoidCrossEntropyRetina(),'txtyloss':nn.L1Loss(),'twth_loss':nn.L1Loss()}
    loss_weight = {'loc_weight':1.0, 'cls_weight':1.0, 'cen_weight':1.0}
    n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print('number of params:', n_parameters)

    actor = actors.siamyolov1Actor(net=model, objective=objective, loss_weight=loss_weight)

    # Optimizer
    param_dicts = [
        {"params": filter(lambda x: x.requires_grad, model.module.backbone.parameters()), "lr":1e-3},
        {"params": model.module.neck_z.parameters()},
        {"params": model.module.neck_x.parameters()},
        {"params": model.module.adjust_weight.parameters()},
