...
class BackboneBase(nn.Module):
    ...
    def __init__(self, backbone: nn.Module, num_channels):
        super().__init__()
        self.body = backbone
...
def build_backbone(settings, backbone_pretrained=True):
    position_embedding = build_position_encoding(settings)
    backbone = Backbone(output_layers=['layer3'], pretrained=backbone_pretrained)
    model = Joiner(backbone, position_embedding)
    model.num_channels = backbone.num_channels
    model.strides = backbone.stride
    return model
